...
function showStory(props) {
    ...
    const { singleStory ,deleteStory,showStory, storyToEdit,setEditedStory,addLike, removeLike, deleteComment,addComment} = storyContext;
    // const { _id} = stories;
    ...
    return (
        ...
        <div className="showStory">
            ...
            <br />
                ...
                <span className="like">
                  <i className="fa fa-thumbs-up" onClick={onLike} />{singleStory.likes && <span>&nbsp;{singleStory.likes.length}</span> } 
                            </span>
                        
                        
                      
                <span >
                        <i className="fas fa-comment" />{singleStory.comments && <span> &nbsp;{singleStory.comments.length}
                        </span>} </span>
                <span className="emojisClass">
                  <i className="far fa-smile-beam" /> : {singleStory.emojis && <span> &nbsp;
                  {singleStory.emojis.map((emoj, id) => (
                    ...
                    <CommentForm />
                    <div >
                        {singleStory.comments && <div>
                            {singleStory.comments.map((comment) => (<div className="post bg-white p-1 my-1">
                                <div>
                                    <Link to={`/profile/`}>
                                ...
                                <div>
                                    ...
                                    <p className="post-date">Posted on {new Intl.DateTimeFormat().format(new Date(comment.date))}</p>
                                    {/* {!auth.loading && user === auth.user._id && ( */}
                                    <button
                                        onClick={() => deleteComment(singleStory._id, comment._id)}
                                        type="button"
